...
exports.pruneFeedDaily = exports.purchaseCoupon = exports.onTaskWrite = void 0;
...
exports.onTaskWrite = functions.firestore
    ...
    const claimedBy = newData.claimedBy;
    if (!claimedBy) {
        functions.logger.error(`Task ${taskId} completed but has no claimedBy uid.`);
        return null;
    }
    const xpReward = (_a = newData.xpReward) !== null && _a !== void 0 ? _a : 0;
    const coinReward = (_b = newData.coinReward) !== null && _b !== void 0 ? _b : 0;
    ...
    const currentLevel = (_c = userData.level) !== null && _c !== void 0 ? _c : 1;
    const currentXp = (_d = userData.currentXp) !== null && _d !== void 0 ? _d : 0;
    const displayName = (_e = userData.displayName) !== null && _e !== void 0 ? _e : "A member";
    ...
    if (newData.isRecurring === true) {
        const recurringRef = db
            ...
            .collection(TASKS_SUB)
            .doc();
        batch.set(recurringRef, Object.assign(Object.assign({}, newData), { taskId: recurringRef.id, status: TASK_STATUS.OPEN, claimedBy: null, proofImageUrl: null, completedAt: null, createdAt: admin.firestore.FieldValue.serverTimestamp() }));
    }
    await batch.commit();
    functions.logger.info(`Task ${taskId} completed by ${claimedBy}. ` +
        `XP: +${xpReward}, Coins: +${coinReward}, LevelUp: ${didLevelUp}`);
    // Send FCM push to the household (best-effort, non-blocking)
    await sendTaskCompletionNotification(householdId, displayName, newData.title, claimedBy);
    return null;
});
...
exports.purchaseCoupon = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be signed in.");
    }
    const uid = context.auth.uid;
    const { householdId, couponId } = data;
    ...
    const userHouseholdId = (_a = buyerSnap.data()) === null || _a === void 0 ? void 0 : _a.householdId;
    ...
    const transaction = db.runTransaction(async (tx) => {
        ...
        const balance = (_a = userData.coinBalance) !== null && _a !== void 0 ? _a : 0;
        const cost = (_b = couponData.cost) !== null && _b !== void 0 ? _b : 0;
        const buyerId = couponData.buyerId;
        const displayName = (_c = userData.displayName) !== null && _c !== void 0 ? _c : "A member";
        if (buyerId != null) {
...
exports.pruneFeedDaily = functions.pubsub
    .schedule("0 3 * * *")
    .timeZone("UTC")
    .onRun(async () => {
    const cutoff = admin.firestore.Timestamp.fromDate(new Date(Date.now() - FEED_RETENTION_DAYS * 24 * 60 * 60 * 1000));
    ...
    const prunePromises = householdsSnap.docs.map(async (householdDoc) => {
        ...
        functions.logger.info(`Pruned ${oldEntries.size} old feed entries from household ${householdDoc.id}`);
    });
...
async function sendTaskCompletionNotification(householdId, actorName, taskTitle, actorUid) {
    var _a;
    try {
        ...
        if (!householdSnap.exists)
            return;
        const memberUids = (_a = householdSnap.data().members) !== null && _a !== void 0 ? _a : [];
        // Fan out to get all FCM tokens, excluding the actor
        const tokenFetches = memberUids
            .filter((uid) => uid !== actorUid)
            .map((uid) => db.collection(USERS_COLLECTION).doc(uid).get().then((snap) => {
            var _a;
        ...
        functions.logger.info(`FCM sent: ${response.successCount} success, ${response.failureCount} failures`);
    }
    catch (err) {
